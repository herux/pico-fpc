...
type
  TI2C_HW = packed record
    ...
    _pad1: array[0..2] of LongWord; { 0x24..0x2C }
    ...
    _pad2: array[0..6] of LongWord; { 0x84..0x9C }
    fs_spklen: LongWord;          { 0xA0 }
  end;
  PI2C_HW = ^TI2C_HW;

var
  i2c0_hw: PI2C_HW absolute I2C0_BASE;
  i2c1_hw: PI2C_HW absolute I2C1_BASE;
...
begin
  i2c^.enable := 0;
  t := 0;
  while ((i2c^.enable and 1) <> 0) and (t < I2C_TRANSFER_TIMEOUT) do
    Inc(t);
end;
...
begin
  ...
  begin
    i2c^.con := I2C_IC_CON_MASTER_MODE_BITS or
                I2C_IC_CON_SPEED_STD_BITS or
                I2C_IC_CON_IC_RESTART_EN_BITS or
                I2C_IC_CON_IC_SLAVE_DISABLE_BITS;
    i2c^.ss_scl_hcnt := hcnt;
    i2c^.ss_scl_lcnt := lcnt;
  end;

  i2c^.rx_tl := 0;
  i2c^.tx_tl := 0;
  i2c^.intr_mask := 0;
  i2c^.clr_intr := i2c^.clr_intr;
  i2c_enable(i2c);

  if use_fast then
    Result := freq_in div (i2c^.fs_scl_hcnt + i2c^.fs_scl_lcnt)
  else
    Result := freq_in div (i2c^.ss_scl_hcnt + i2c^.ss_scl_lcnt);
end;
...
begin
  ...
  begin
    ...
    if (i = len - 1) and (not nostop) then
  ...
  wait_count := 0;
  while (i2c^.status and I2C_IC_STATUS_MST_ACTIVITY_BITS) <> 0 do
  begin
    if (i2c^.raw_intr_stat and I2C_IC_RAW_INTR_STAT_TX_ABRT_BITS) <> 0 then
    begin
      i2c^.clr_tx_abrt := i2c^.clr_tx_abrt;
